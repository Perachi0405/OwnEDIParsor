...
func (r *ediReader) Read() (*idr.Node, error) {
	...
	for {
		...
		cur := r.stackTop() //{0xc000afa770 0xc000094300 1 0}
		...
		if !cur.segDecl.isGroup() { //executes after r.stackTop
			...
			cur.segNode = idr.CreateNode(idr.ElementNode, cur.segDecl.Name) //executes after isGroup()
		}
		if len(r.stack) > 1 {
...
func NewReader(inputName string, r io.Reader, decl *FileDecl, targetXPath string) (*ediReader, error) {
	...
	reader := &ediReader{
		inputName:         inputName,
		r:                 NewNonValidatingReader(r, decl), //input the reader and fileDeclaration
