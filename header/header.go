...
func (p ParserSettings) WrapEncoding(input io.Reader) io.Reader {
	// var samp = supportedEncodingMappings[encodingISO8859_1]
	// dec := hex.DecodeString(samp)
	fmt.Println("header.go supportedEncodingMappings", supportedEncodingMappings)
	fmt.Println("supportedEncodingMappings", supportedEncodingMappings[encodingISO8859_1])
	f, found := supportedEncodingMappings[strs.StrPtrOrElse(p.Encoding, encodingUTF8)]
	fmt.Println("Header.go f", f)
	if !found {
